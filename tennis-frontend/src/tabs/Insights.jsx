...
// completa, tipear la filtra. Mismo patron que el resto del dashboard.
  ...
  return (
    <div ref={ref} style={{ position: "relative" }}>
      ...
      <input
        ...
        placeholder="Click or type to highlight..."
        style={{
...
export default function Insights({ data }) {
  const { players, eloCurrent, eloHistory, playerSurfaceStatsByYear, playerTitles, h2hOverall, currentRanking, bestCareerRank } = data;
  const [tourFilter, setTourFilter] = useState("ALL");
  const tourLabel = tourFilter === "ALL" ? "ATP + WTA" : tourFilter;

  const [insightsPlayerQuery, setInsightsPlayerQuery] = useState("");
  ...
  const currentRankByPlayer = useMemo(() => new Map(currentRanking.map((r) => [r.player, r.current_rank])), [currentRanking]);
  const bestRankByPlayer = useMemo(() => new Map(bestCareerRank.map((r) => [r.player, r.best_rank])), [bestCareerRank]);
  ...
  // restringido a lo que ya es elegible con el Tour/Ranking status
  // elegidos arriba.
  const sortedEligiblePlayerNames = useMemo(() => {
  ...
  }, [eloCurrent, tourFilter, currentRankByPlayer, bestRankByPlayer, rankStatusFilter]);
    ...
    <div>
      <div style={{ display: "flex", gap: 20, alignItems: "center", marginBottom: 16, flexWrap: "wrap" }}>
        ...
        <PlayerSearchCombobox
          players={sortedEligiblePlayerNames}
          value={insightsPlayerQuery}
          onChange={setInsightsPlayerQuery}
        />
        <div>
        ...
        <ResponsiveContainer width="100%" height={320}>
          <BarChart data={eloLeaderboard} layout="vertical" margin={{ left: 10 }}>
            ...
            <Bar dataKey="elo" radius={[0, 4, 4, 0]}>
              {eloLeaderboard.map((r, i) => (
                <Cell
                  key={i}
                  fill={
                    insightsPlayerQuery.trim() && r.player.toLowerCase() === insightsPlayerQuery.trim().toLowerCase()
                      ? "#FFFFFF"
                      : ELO_COLOR
                  }
                />
